...
const UPDATE_POST = gql`
  mutation CREATE_POST($title: String, $description: String, $costs: [Costcalculate!], $postId: ID!) {
    updatePost(title: $title, description: $description, costs: $costs, postId: $postId) {
      id
      costs {
        title
        price
...
class UpdatePost extends React.Component {
  ...
  render() {
    ...
    return (
      <Query query={SINGLE_ITEM_QUERY} variables={{ id: this.props.postId }}>
        {({ data, loading }) => {
          ...
          return (
            <Wrapper>
              ...
              <Mutation mutation={UPDATE_POST} variables={{ ...this.state, postId: this.props.postId }} refetchQueries={[{ query: ALL_POSTS_QUERY }]}>
                      ...
                      <Row>
                        <Col md={10} className='mx-auto'>
                          <Form
                            ...
                            onSubmit={async e => {
                              e.preventDefault()
                              let costs = this.state.costs || data.post.costs
                              costs.map(cost => {
                                if (cost.price === undefined) {
                                  cost.price = 0
                                }
                                if (cost.__typename) delete cost.__typename
                              })
                              this.setState({ costs })
                            ...
                            {data.post.costs.map((cost, index) => {
                              return (
                                <Row form key={`input_${index}`}>
                                  ...
                                  <Col md={1} key={`button_${index}`}>
                                    {data.post.costs.length > 1 || costs.length > 1 ? (
                                      <button
                                        className='close-button'
                                        onClick={e => {
                                          e.preventDefault()
                                          let costs = this.state.costs ? this.state.costs : data.post.costs
